...
welcoming contributions from researchers (please open an issue and we'll discuss!).
...
In the sample dataset, these are the columns `cases` (number of dengue cases), `date` (temporal identifier), and `regional_geocode` (spatial identifier).
...
Note that `epiyearweek` lets you choose whether epidemiological weeks start on a Sunday or a Monday via the `start` argument. 
...
Note that for epiyears where not all weeks are available or there are missing cases, the result will be NA, e.g. at the borders
